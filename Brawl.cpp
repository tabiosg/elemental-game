...
void Brawl::startBrawl() {
	...
	while (team1_size != 0 && team2_size != 0) {
		...
		std::string dummy;
		std::cout << std::endl << "Type anything to see the next move. ";
		std::cin >> dummy;
		std::cout << std::endl;
		...
		if (is_team_1_turn) {
			std::cout << "Team 1 will send out its fighter! " << std::endl << std::endl;
			...
			std::cout << "Team 2 will send out its fighter! " << std::endl << std::endl;
	...
	team2_size = team2.get_in_combat_size();
	if (team1_size == 0) {
		std::cout << "Team 1 wins!" << std::endl << std::endl;
	}
	else {
		std::cout << "Team 2 wins!" << std::endl << std::endl;
	}
	team1.exit_brawl();
	team2.exit_brawl();
}
...
void Brawl::request_and_enact_action(Fighter* f, Team& allies, Team& opponents) {
	std::cout << "It is " << *f << "'s turn." << std::endl << std::endl;
	...
	if (response == "Attack") {
		...
		std::cout << *f << " skips their turn." << std::endl << std::endl;
	}
}
